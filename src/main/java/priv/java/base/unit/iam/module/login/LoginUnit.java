...
import priv.java.base.unit.iam.common.annotation.LoginCheck;
import priv.java.base.unit.iam.common.annotation.ActionCheck;
import priv.java.base.unit.iam.common.annotation.RoleCheck;
import priv.java.base.unit.iam.common.annotation.CheckManner;
...
public class LoginUnit {
	 ...
	 */
	public LoginUnit(String loginKey) {
	 ...
	 * @return 对象自身
	 */
	public LoginUnit setLoginKey(String loginKey){
		this.loginKey = loginKey;
		return this;
	}
	 ...
	 */
 	public String createTokenValue(Object loginId) {
		return IAMComponentFactory.getSaTokenAction().createToken(loginId, loginKey);
	}
	...
	public String getTokenValue(){
		// 0. 获取相应对象 
		HttpServletRequest request = IAMComponentFactory.getSaTokenServlet().getRequest();
		...
		if(tokenValue == null && config.getIsReadCookie()){
			Cookie cookie = IAMComponentFactory.getSaTokenCookie().getCookie(request, keyTokenName);
			if(cookie != null){
	 ...
	 * @return token信息 
	 */
	public TokenCard getTokenInfo() {
		TokenCard info = new TokenCard();
	...
	public void setLoginId(Object loginId) {
		setLoginId(loginId, new LoginLicense());
	}
	...
	public void setLoginId(Object loginId, String device) {
		setLoginId(loginId, new LoginLicense().setDevice(device));
	}
	...
	public void setLoginId(Object loginId, boolean isTempCookie) {
		setLoginId(loginId, new LoginLicense().setIsTempCookie(isTempCookie));
	}
	 ...
	 */
	public void setLoginId(Object loginId, LoginLicense loginModel) {
		
		// ------ 1、获取相应对象  
		HttpServletRequest request = IAMComponentFactory.getSaTokenServlet().getRequest();
		TokenConfig config = getConfig();
		TokenService dao = IAMComponentFactory.getSaTokenDao();
		...
		if(config.getIsReadCookie() == true){
			HttpServletResponse response = IAMComponentFactory.getSaTokenServlet().getResponse();
			int cookieTimeout = loginModel.getIsTempCookie() ? -1 : (int)(long)loginModel.getTimeout();
			IAMComponentFactory.getSaTokenCookie().addCookie(response, getTokenName(), tokenValue,
	...
	public void logout() {
		...
		String tokenValue = getTokenValue();
 		...
 		if(getConfig().getIsReadCookie() == true){
			IAMComponentFactory.getSaTokenCookie().delCookie(IAMComponentFactory.getSaTokenServlet().getRequest(), IAMComponentFactory.getSaTokenServlet().getResponse(), getTokenName());
		}
	...
	public void logoutByTokenValue(String tokenValue) {
		...
		clearLastActivity(tokenValue); 	
 		...
 		IAMComponentFactory.getSaTokenDao().delete(splicingKeyTokenValue(tokenValue));
	...
	public void logoutByLoginId(Object loginId, String device) {
		...
		Session session = getSessionByLoginId(loginId);
		if(session == null) {
			return;
		}
		...
		for (TokenSign tokenSign : tokenSignList) {
			if(device == null || tokenSign.getDevice().equals(device)) {
				...
				clearLastActivity(tokenValue); 	
		 		// 3. 标记：已被踢下线 
				IAMComponentFactory.getSaTokenDao().update(splicingKeyTokenValue(tokenValue), NotLoginException.KICK_OUT);
	...
	public Object getLoginIdDefaultNull() {
		...
		String tokenValue = getTokenValue();
 		...
 		// 如果已经[临时过期] 
 		if(getTokenActivityTimeoutByToken(tokenValue) == TokenService.NOT_VALUE_EXPIRE) {
 			return null;
 		}
	 ...
	 */
 	...
 	public String getLoginIdNotHandle(String tokenValue) {
 		return IAMComponentFactory.getSaTokenDao().get(splicingKeyTokenValue(tokenValue));
 	}
	...
	public Session getSessionBySessionId(String sessionId, boolean isCreate) {
		Session session = IAMComponentFactory.getSaTokenDao().getSession(sessionId);
		if(session == null && isCreate) {
			session = IAMComponentFactory.getSaTokenAction().createSession(sessionId);
			IAMComponentFactory.getSaTokenDao().setSession(session, getConfig().getTimeout());
		}
		return session;
	}
	...
	public Session getTokenSession(boolean isCreate) {
		...
		if(getConfig().getTokenSessionCheckLogin()) {
			...
			if(tokenValue == null || Objects.equals(tokenValue, "")) {
				...
				// Request做上标记 
				IAMComponentFactory.getSaTokenServlet().getRequest().setAttribute(splicingKeyJustCreatedSave(), tokenValue);
				// 写入 [最后操作时间]
				setLastActivityToNow(tokenValue);  
				// cookie注入 
				if(getConfig().getIsReadCookie() == true){	
					IAMComponentFactory.getSaTokenCookie().addCookie(IAMComponentFactory.getSaTokenServlet().getResponse(), getTokenName(), tokenValue,
	...
	/**
 	...
 	protected void setLastActivityToNow(String tokenValue) {
 		...
 		if(tokenValue == null || getConfig().getActivityTimeout() == TokenService.NEVER_EXPIRE) {
 			return;
 		}
 		// 将[最后操作时间]标记为当前时间戳 
 		IAMComponentFactory.getSaTokenDao().set(splicingKeyLastActivityTime(tokenValue), String.valueOf(System.currentTimeMillis()), getConfig().getTimeout());
 	}
 	...
 	protected void clearLastActivity(String tokenValue) {
 		...
 		// 删除[最后操作时间]
 		IAMComponentFactory.getSaTokenDao().delete(splicingKeyLastActivityTime(tokenValue));
 		// 清除标记 
 		IAMComponentFactory.getSaTokenServlet().getRequest().removeAttribute(AppConst.TOKEN_ACTIVITY_TIMEOUT_CHECKED_KEY);
 	}
 	...
 	public void checkActivityTimeout(String tokenValue) {
 		...
 		HttpServletRequest request = IAMComponentFactory.getSaTokenServlet().getRequest();
 		if(request.getAttribute(AppConst.TOKEN_ACTIVITY_TIMEOUT_CHECKED_KEY) != null) {
 			return;
 		}
 		// ------------ 验证是否已经 [临时过期] 
 		// 获取 [临时剩余时间]
 		...
 		if(timeout == TokenService.NEVER_EXPIRE) {
 			return;
 		}
 		// -2 代表已过期，抛出异常 
 		if(timeout == TokenService.NOT_VALUE_EXPIRE) {
 			throw NotLoginException.newInstance(loginKey, NotLoginException.TOKEN_TIMEOUT);
 		}
 		// --- 至此，验证已通过 

 		// 打上[检查标记]，标记一下当前请求已经通过验证，避免一次请求多次验证，造成不必要的性能消耗 
 		request.setAttribute(AppConst.TOKEN_ACTIVITY_TIMEOUT_CHECKED_KEY, true);
 	}
 	...
 	public void updateLastActivityToNow(String tokenValue) {
 		...
 		IAMComponentFactory.getSaTokenDao().update(splicingKeyLastActivityTime(tokenValue), String.valueOf(System.currentTimeMillis()));
 	}
	...
	// =================== 过期时间相关 ===================  
 	...
 	public long getTokenTimeout() {
 		return IAMComponentFactory.getSaTokenDao().getTimeout(splicingKeyTokenValue(getTokenValue()));
 	}
 	...
 	public long getTokenTimeoutByLoginId(Object loginId) {
 		return IAMComponentFactory.getSaTokenDao().getTimeout(splicingKeyTokenValue(getTokenValueByLoginId(loginId)));
 	}
 	...
 	public long getSessionTimeoutByLoginId(Object loginId) {
 		return IAMComponentFactory.getSaTokenDao().getSessionTimeout(splicingKeySession(loginId));
 	}
 	...
 	public long getTokenSessionTimeoutByTokenValue(String tokenValue) {
 		return IAMComponentFactory.getSaTokenDao().getSessionTimeout(splicingKeyTokenSession(tokenValue));
 	}
 	...
 	public long getTokenActivityTimeoutByToken(String tokenValue) {
 		...
 		// 如果设置了永不过期, 则返回 -1 
 		if(getConfig().getActivityTimeout() == TokenService.NEVER_EXPIRE) {
 			return TokenService.NEVER_EXPIRE;
 		}
 		// ------ 开始查询 
 		// 获取相关数据 
 		String keyLastActivityTime = splicingKeyLastActivityTime(tokenValue);
 		String lastActivityTimeString = IAMComponentFactory.getSaTokenDao().get(keyLastActivityTime);
 		// 查不到，返回-2 
	...
	// =================== 角色验证操作 ===================  
 	...
 	public boolean hasRole(Object loginId, String role) {
 		List<String> roleList = IAMComponentFactory.getStpInterface().getRoleList(loginId, loginKey);
	...
	// =================== 权限验证操作 ===================  
 	...
 	public boolean hasPermission(Object loginId, String permission) {
 		List<String> permissionList = IAMComponentFactory.getStpInterface().getPermissionList(loginId, loginKey);
	...
	public String getTokenValueByLoginId(Object loginId) {
		return getTokenValueByLoginId(loginId, AppConst.DEFAULT_LOGIN_DEVICE);
	}
	...
	public List<String> getTokenValueListByLoginId(Object loginId) {
		return getTokenValueListByLoginId(loginId, AppConst.DEFAULT_LOGIN_DEVICE);
	}
	...
	public List<String> searchTokenValue(String keyword, int start, int size) {
		return IAMComponentFactory.getSaTokenDao().searchData(splicingKeyTokenValue(""), keyword, start, size);
	}
	...
	public List<String> searchSessionId(String keyword, int start, int size) {
		return IAMComponentFactory.getSaTokenDao().searchData(splicingKeySession(""), keyword, start, size);
	}
	...
	public List<String> searchTokenSessionId(String keyword, int start, int size) {
		return IAMComponentFactory.getSaTokenDao().searchData(splicingKeyTokenSession(""), keyword, start, size);
	}
	...
	public String splicingKeySwitch() {
		return AppConst.SWITCH_TO_SAVE_KEY + getLoginKey();
	}
	...
	public String splicingKeyJustCreatedSave() {
		return AppConst.JUST_CREATED_SAVE_KEY + getLoginKey();
	}
	...
	public TokenConfig getConfig() {
		...
		return IAMComponentFactory.getConfig();
	}
	...
	public void checkMethodAnnotation(Method method) {
		...
		// ----------- 验证登录 
		if(method.isAnnotationPresent(LoginCheck.class) || method.getDeclaringClass().isAnnotationPresent(LoginCheck.class)) {
			this.checkLogin();
		}
		
		// ----------- 验证角色
		// 验证方法上的 
		RoleCheck scr = method.getAnnotation(RoleCheck.class);
		if(scr != null) { 
			String[] roleArray = scr.value();
			if(scr.mode() == CheckManner.AND) {
		...
		// 验证类上的 
		scr = method.getDeclaringClass().getAnnotation(RoleCheck.class);
		if(scr != null) { 
			String[] roleArray = scr.value();
			if(scr.mode() == CheckManner.AND) {
				this.checkRoleAnd(roleArray);	
		...
		// ----------- 验证权限 
		// 验证方法上的 
		ActionCheck scp = method.getAnnotation(ActionCheck.class);
		if(scp != null) { 
			String[] permissionArray = scp.value();
			if(scp.mode() == CheckManner.AND) {
		...
		// 验证类上的 
		scp = method.getDeclaringClass().getAnnotation(ActionCheck.class);
		if(scp != null) { 
			String[] permissionArray = scp.value();
			if(scp.mode() == CheckManner.AND) {
				this.checkPermissionAnd(permissionArray);	
	...
	public void switchTo(Object loginId) {
		IAMComponentFactory.getSaTokenServlet().getRequest().setAttribute(splicingKeySwitch(), loginId);
	}
	...
	public void endSwitch() {
		IAMComponentFactory.getSaTokenServlet().getRequest().removeAttribute(splicingKeySwitch());
	}
	...
	public boolean isSwitch() {
		return IAMComponentFactory.getSaTokenServlet().getRequest().getAttribute(splicingKeySwitch()) != null;
	}
	...
	public Object getSwitchLoginId() {
		return IAMComponentFactory.getSaTokenServlet().getRequest().getAttribute(splicingKeySwitch());
	}
	 ...
	 */
	public void switchTo(Object loginId, Runnable function) {
		try {
			switchTo(loginId);
			function.run();
		} catch (Exception e) {
